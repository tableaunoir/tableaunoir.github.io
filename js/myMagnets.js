...
function createMagnetRainbow(content) {
    var o = createMagnet(content);
    var colors = ['rgb(139, 97, 195)', 'rgb(115, 97, 195)', 'rgb(93, 105, 214)', 'rgb(40, 167, 226)', 'rgb(40, 204, 226)', 'rgb(40, 226, 201)', 'rgb(40, 226, 148)',
...
function magnetGraphNodes() {
    //MagnetManager.clearMagnet();
    for (var _i = 0, _a = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O']; _i < _a.length; _i++) {
        var i = _a[_i];
...
function magnetGraphSimCity() {
    //MagnetManager.clearMagnet();
    var simCityPictures = ["antenne.png", "commerce.png", "parking.png", "tour.png",
    ...
    for (var _i = 0, simCityPictures_1 = simCityPictures; _i < simCityPictures_1.length; _i++) {
        var name_1 = simCityPictures_1[_i];
        MagnetManager.addMagnetImage("simCityGraph/" + name_1);
    }
}
/** Tilings */
...
function magnetGo() {
    MagnetManager.clearMagnet();
    var f = function (color, x) {
        var _loop_1 = function (i) {
            MagnetManager.addMagnetImage("go/" + color + ".png", function (img) { return Share.execute("magnetMove", [img.id, x, 10 + i * 5]); });
        };
        for (var i = 0; i < 20; i++) {
            _loop_1(i);
        }
    ...
    f("black", 20);
    f("white", 50);
    MagnetManager.addMagnetImage("go/goban.png", function (img) { return Share.execute("magnetMove", [img.id, 110, 20]); });
}
...
//# sourceMappingURL=myMagnets.js.map